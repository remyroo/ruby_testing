...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:continue_game) { described_class.new(4, '6') }
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_three) { described_class.new(2, '2', 5) }

      it 'outputs correct phrase' do
        other_phrase = "That was hard. It took you 5 guesses!\n"
        expect { game_three.final_message }.to output(other_phrase).to_stdout
      end
    end
  end
end
